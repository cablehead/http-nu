...
async fn test_dynamic_script_reload() {
    ...
    let mut server = TestServer::new_with_stdin("127.0.0.1:0", false).await;

    // Write initial script: returns "1" after 100ms delay
    server.write_script("{|req| sleep 100ms; \"1\"}").await;
