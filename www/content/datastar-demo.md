...
Server-side: `from datastar-request` extracts signals (like interval).
...
`to dstar-patch-element` for DOM updates.

Finally, `append` adds a `to dstar-patch-signal` to set `running=false`,
