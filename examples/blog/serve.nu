...
def post-detail [req slug: string] {
  ...
  page-layout $req $post.title (MAIN {class: "post-content"}
  (ARTICLE
  (H2 $post.title)
  (P {class: "meta"} $post.date)
...
{|req|
  dispatch $req [
    ...
    # 404 fallback
    (route true {|req ctx|
      page-layout $req "Not Found" (MAIN
      (H2 "Page Not Found")
      (P "The page you're looking for doesn't exist.")
      (P (A {href: ($req | href "/")} "<- Back to home"))) | metadata set { merge {'http.response': {status: 404}} }
    })
  ]
}
