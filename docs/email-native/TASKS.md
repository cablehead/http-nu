...
- [x] **#23** Paid-tier smoke -- **PASSED end-to-end 2026-05-26 against gedw99@gmail.com via `mail.amplify-cms.com`**. Outbound-only mode of `examples/email-demo/test/smoke.nu` returned `result=delivered` with a CF message_id, AND the user confirmed inbox receipt. Inbound half deferred until http-nu is deployed publicly (see #22). No dashboard click needed -- full chain was scripted thanks to the API discovery codified in `email:sender-domain:add`. Reproducibility verified by full teardown + `email-demo:full` redeploy cycle.
